CREATE OR REPLACE FUNCTION public.atomic_subscribe_copy_trader(p_follower_id uuid, p_trader_id uuid, p_amount numeric, p_auto_copy boolean DEFAULT true, p_max_trade_amount numeric DEFAULT NULL::numeric, p_copy_percentage numeric DEFAULT 100)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Create ledger entry (using meta column instead of description)
  INSERT INTO ledger_entries (
    user_id, wallet_id, amount, net_amount, direction, ref_type, status, meta
  ) VALUES (
    p_follower_id, v_wallet.id, p_amount, p_amount, 'DEBIT', 'COPY_SUBSCRIPTION', 'COMPLETED',
    jsonb_build_object('description', 'Copy Trade subscription: ' || v_copy_trader.display_name, 'trader_id', p_trader_id)
  );
