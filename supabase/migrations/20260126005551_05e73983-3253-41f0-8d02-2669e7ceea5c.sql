-- Drop the position check constraints to allow multi-option trading
-- The position column now uses option labels or YES/NO for binary markets

-- Drop constraint from transactions table
ALTER TABLE public.transactions DROP CONSTRAINT IF EXISTS transactions_position_check;

-- Drop constraint from user_contracts table  
ALTER TABLE public.user_contracts DROP CONSTRAINT IF EXISTS user_contracts_position_check;

-- Also drop the unique constraint that includes position to allow option_id based uniqueness
ALTER TABLE public.user_contracts DROP CONSTRAINT IF EXISTS user_contracts_user_id_market_id_position_key;

-- Add a new unique constraint that uses option_id for multi-option markets
-- For binary markets (option_id is null), position differentiates YES/NO
-- For multi-option markets, option_id differentiates options
CREATE UNIQUE INDEX IF NOT EXISTS user_contracts_unique_position 
ON public.user_contracts (user_id, market_id, COALESCE(option_id::text, position));

-- Update atomic_execute_multi_trade to set position to 'OPTION' for multi-option trades
...
BEGIN
  ...
  INSERT INTO ledger_entries (
    user_id, wallet_id, ref_type, direction, amount, fee_amount, net_amount, platform_revenue, status
  ) VALUES (
    p_user_id, v_wallet.id, 'TRADE', 'OUT', v_trade_cost, 0, v_trade_cost, 0, 'COMPLETED'
  ) RETURNING id INTO v_ledger_id;
  
  -- Transaction record - use 'OPTION' as position for multi-option trades
  ...
  -- User contract (upsert) - use option_id for uniqueness
