-- Fix atomic_execute_trade: replace legacy `contracts` table usage with `user_contracts`
-- This resolves 42P01 (relation "contracts" does not exist) errors triggered by execute-trade.
...
BEGIN
  ...
  -- Upsert user contract (binary markets)
  SELECT * INTO v_existing_contract
  FROM user_contracts
  WHERE user_id = p_user_id
    AND market_id = p_market_id
    AND position = p_outcome
    AND option_id IS NULL
  FOR UPDATE;

  IF FOUND THEN
    UPDATE user_contracts
    SET shares = v_existing_contract.shares + p_shares,
  ...
  ELSE
    INSERT INTO user_contracts (
      user_id, market_id, position, shares, average_price, total_invested, option_id
    ) VALUES (
      p_user_id, p_market_id, p_outcome, p_shares, v_price_per_share, v_trade_cost, NULL
    ) RETURNING id INTO v_contract_id;
  END IF;
